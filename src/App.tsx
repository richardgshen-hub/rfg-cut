...
import { buildCodexEditPrompt, buildCodexNarrativePrompt } from './codexAgent'
...
type Panel = 'highlights' | 'transcript' | 'brief' | 'script' | 'review' | 'agent'
...
function AgentPanel({ session, connected, onJumpToTime }: { session: AgentBridgeSession | null; connected: boolean; onJumpToTime: (seconds: number) => void }) {
  ...
  const narrativePlan = session?.narrativePlan
  ...
  if (narrativePlan) return <div className="agent-panel narrative-result"><div className="agent-status live"><i />叙事整理已回传 · 待复核</div><h2>{narrativePlan.title}</h2><p className="agent-plan-intro">{narrativePlan.summary}</p>{narrativePlan.targetScript && <p className="target-script-note">目标成稿：{narrativePlan.targetScript}</p>}<h2>建议叙事顺序</h2>{narrativePlan.beats.map((beat) => <button className="agent-item narrative" type="button" key={beat.id} onClick={() => onJumpToTime(Math.floor(beat.start))}><span>{formatTime(beat.start).slice(3)} — {formatTime(beat.end).slice(3)} · {beat.role}</span><strong>{beat.sourceText}</strong><small>{beat.editReason}</small></button>)}{narrativePlan.warnings.length > 0 && <><h2>需注意</h2>{narrativePlan.warnings.map((warning) => <p className="narrative-warning" key={warning}>{warning}</p>)}</>}<p className="agent-plan-intro">{narrativePlan.nextStep}</p></div>
...
function ScriptTools({ timestampedScript, targetScript, onTimestampedScriptChange, onTargetScriptChange, onApplyTimestampedScript, onCreateNarrative }: { timestampedScript: string; targetScript: string; onTimestampedScriptChange: (value: string) => void; onTargetScriptChange: (value: string) => void; onApplyTimestampedScript: () => void; onCreateNarrative: () => void }) {
  return <section className="script-tools"><div className="script-tools-head"><h2>文稿剪辑</h2><p>有准备好的逐字稿，就按它找原片；没有时间码的成稿，则交给 Codex 对齐真实口播。</p></div><label><span>准备好的逐字稿（可带时间码）</span><textarea value={timestampedScript} onChange={(event) => onTimestampedScriptChange(event.target.value)} placeholder={'支持 SRT、逐行时间码，或直接粘贴纯文字：\n00:00 - 00:06 我们想做这件事，是因为…'} /></label><button type="button" className="script-button" onClick={onApplyTimestampedScript}><Icon name="captions" size={15} />导入稿件 / 按稿准备</button><label><span>目标成稿 / 想表达的主线</span><textarea value={targetScript} onChange={(event) => onTargetScriptChange(event.target.value)} placeholder="例如：用 60 秒讲清楚为什么做这场展览、它给孩子带来什么，以及希望家长怎么行动。" /></label><button type="button" className="narrative-button" onClick={onCreateNarrative}><Icon name="sparkle" size={15} />交给 Codex 按稿对齐并整理</button><p className="script-safety">结果会列出每段原话的时间码；原片没有说过的内容会标为“需补录 / 字幕补充”，不会由 AI 悄悄编进去。</p></section>
}

function App() {
  const [playing, setPlaying] = useState(false)
  const [time, setTime] = useState(42)
  const [panel, setPanelState] = useState<Panel>('highlights')
  const setPanel = (next: Panel) => setPanelState(next === 'script' ? 'brief' : next)
  ...
  const copyCodexAgentPrompt = async () => {
    const context = { sourceMedia: sourceFile?.name ?? '尚未导入素材', briefing, transcript: transcriptLines, hasActualTranscript, targetScript }
  ...
  const applyTimestampedScript = () => {
    try {
      const parsed = parseTimestampedTranscript(timestampedScript)
      setTranscriptLines(parsed)
      setSuggestions(createHighlights(parsed))
      setHasActualTranscript(true)
      setPanel('review')
      setNotice(`已导入 ${parsed.length} 段带时间码逐字稿，可按稿生成剪辑计划`)
    } catch (error) {
      if (timestampedScript.trim()) {
        setTargetScript(timestampedScript.trim())
        setPanel('script')
        setNotice('已保存纯文字目标稿；有真实转写后可交给 Codex 按稿对齐')
      } else setNotice(error instanceof Error ? error.message : '逐字稿格式无法解析')
    }
  }
  const copyNarrativePrompt = async () => {
    if (!hasActualTranscript) { setNotice('请先完成本地转写，或导入带时间码逐字稿；纯文字稿不能直接生成切点。'); return }
    const context = { sourceMedia: sourceFile?.name ?? '尚未导入素材', briefing, transcript: transcriptLines, hasActualTranscript, targetScript }
    const prompt = buildCodexNarrativePrompt(context)
    const bridgeRequest = publishAgentContext(context)
    try {
      const [, session] = await Promise.all([navigator.clipboard.writeText(prompt), bridgeRequest])
      bridgeRevision.current = session.revision
      ...
      setAgentConnected(true)
      setPanel('agent')
      setNotice('已同步叙事整理任务：粘贴给 Codex，计划会回到此处')
    } catch {
      try { await navigator.clipboard.writeText(prompt); setNotice('已复制叙事整理任务；启动 npm run agent:bridge 后可自动回传') } catch { setNotice('复制失败：请在支持剪贴板权限的浏览器中重试') }
    }
  }
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (!cancelled && next.revision !== bridgeRevision.current) {
          ...
          if (next.reviewPlan || next.narrativePlan) setPanel('agent')
        }
      } catch {
